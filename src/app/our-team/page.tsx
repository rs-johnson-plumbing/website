        ...
        <div className="flex flex-col items-start gap-4">
          ...
          {member.badges && (
            <ul className="flex flex-wrap gap-2">
              {member.badges.map((b) => (
                <li key={b} className="inline-flex items-center gap-1.5 rounded-btn bg-teal-tint px-3 py-1.5 text-[13px] font-bold text-teal-dark">
            ...
            return (
              <div key={k} className="rounded-card border border-hairline bg-white p-5 lg:p-6">
                <h3 className="inline-flex items-center gap-2 text-[19px] font-bold lg:text-[20px]">
                  <Icon name={k === "homeowners" ? "house" : "hammer"} size={20} strokeWidth={1.8} className="text-teal" />
                  {block.heading}
                </h3>
                <ul className="mt-4 flex flex-col gap-3">
                  {block.items.map((item) => (
                    <li key={item} className="flex items-start gap-2.5 text-[16px] leading-snug">
                      <Icon name="check" size={18} strokeWidth={2.2} className="mt-0.5 shrink-0 text-teal" />
                      {item}
                    </li>
                  ))}
                </ul>
