...
export async function POST(request:NextRequest){
  ...
  if(process.env.ROBO_RYAN_AI_ENABLED!=='true'||!process.env.OPENAI_API_KEY||!process.env.OPENAI_MODEL)return NextResponse.json({reply:'Live answers are not connected in this preview yet. You can try the Repair or Install questions, or call the team at 314-220-1827.',preview:true});
  ...
  try {
    const response=await fetch('https://api.openai.com/v1/responses',{method:'POST',headers:{Authorization:`Bearer ${process.env.OPENAI_API_KEY}`,'Content-Type':'application/json'},body:JSON.stringify({model:process.env.OPENAI_MODEL,instructions,input:messages as ChatMessage[],store:false,max_output_tokens:500,...(process.env.OPENAI_VECTOR_STORE_ID?{tools:[{type:'file_search',vector_store_ids:[process.env.OPENAI_VECTOR_STORE_ID]}]}:{})}),signal:AbortSignal.timeout(22000)});
