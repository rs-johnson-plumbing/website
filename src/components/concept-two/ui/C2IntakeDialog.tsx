...
function Field({ name, label, fields, change, required = false, type = "text", autoComplete, pattern }: {
  ...
  return <label><span>{label}{required ? " *" : ""}</span><input name={name} type={type} required={required} autoComplete={autoComplete} pattern={pattern} maxLength={254} value={fields[name] || ""} onChange={event => change(name, event.target.value)} /></label>;
}
      ...
      <div className="ci-body" ref={body}>
        {status === "done" ? <><h2 tabIndex={-1}>Thank you.</h2><p>Your request has been received.</p>{!isBid && <p>Your preferred arrival window still needs confirmation.</p>}<C2Button onClick={onClose}>Done</C2Button></> : <>
          {isBid ? <>
            ...
            {step === 0 && <><h2 tabIndex={-1}>Where do you need service?</h2><p>Serving St. Charles and St. Louis.</p><Field name="zip" label="ZIP code" required pattern="[0-9]{5}" fields={fields} change={change} autoComplete="postal-code" /></>}
