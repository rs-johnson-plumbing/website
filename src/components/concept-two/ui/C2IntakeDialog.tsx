      ...
      <div className="ci-body" ref={body}>
        {status === "done" ? <><h2 tabIndex={-1}>Thank you.</h2><p>Your request has been received.</p>{!isBid && <p>Your preferred arrival window still needs confirmation.</p>}<C2Button onClick={onClose}>Done</C2Button></> : <>
          {isBid ? <>
            ...
            {step === 1 && <><h2 tabIndex={-1}>What can we help you with?</h2><div className="ci-categories" role="group" aria-label="Service type">{categories.map(category => <button type="button" key={category.name} aria-pressed={fields.category === category.name} onClick={() => setFields(old => ({ ...old, category: category.name, issue: "", details: "" }))}><ServiceSketch id={category.art} /><span>{category.name}</span></button>)}</div>
