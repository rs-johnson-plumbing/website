...
/** Sand circle behind an icon, as in the trust bar. */
export function IconCircle({ name, size = 34 }: { name: IconName; size?: number }) {
  return (
    <span
      className="flex shrink-0 items-center justify-center rounded-full bg-sand text-charcoal"
...
/** Sand rounded square behind an icon, as on service cards. */
export function IconTile({ name, size = 40 }: { name: IconName; size?: number }) {
  return (
    <span
      className="flex shrink-0 items-center justify-center rounded-tile bg-sand text-charcoal"
      style={{ width: size, height: size }}
    >
