 ...
 * Two-tone by default (charcoal pipe, copper flanges); pass `reversed`
...
export function Mark({ className, title, reversed = false }: { className?: string; title?: string; reversed?: boolean }) {
  ...
  return (
    <svg viewBox="0 0 200 240" fill="none" aria-hidden={title ? undefined : true} role={title ? "img" : undefined} className={className}>
      ...
      <rect x="112" y="6" width="56" height="18" rx="2" fill="#A85A2E" />
      <rect x="116" y="112" width="48" height="14" rx="2" fill="#A85A2E" />
      <rect x="8" y="90" width="16" height="48" rx="2" fill="#A85A2E" />
    </svg>
  );
}
