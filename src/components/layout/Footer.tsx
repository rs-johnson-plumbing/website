      ...
      <div className="site-width gutter pb-8 pt-14 lg:pt-16">
        <div className="mb-10 grid grid-cols-1 gap-10 sm:grid-cols-2 lg:grid-cols-3">
          <div className="flex flex-col items-start gap-2 text-[15px]">
            <Logo size="footer" />
            <div className="text-slate">{site.basedIn}</div>
            <div className="mt-1">
              <a href={site.phone.tel} data-track="call-footer" className="text-charcoal hover:underline">
                {site.phone.display}
              </a>
              <span className="text-slate"> · {site.phone.note}</span>
            </div>
            <div className="text-charcoal">{site.email}</div>
            <div className="mt-3 text-[15px] font-bold">{site.footer.reviewsHeading}</div>
            <ul className="flex flex-col gap-2 text-slate">
              {reviewLinks.map((r) => (
                <li key={r.label}>
          ...
          <div>
            ...
            <ul className="flex flex-col gap-2 text-[15px] text-slate">
              {services.map((s) => (
                <li key={s.slug}>
                  <Link href={`/plumbing/${s.slug}`} className="hover:text-charcoal hover:underline">
                    {s.name}
                  </Link>
                </li>
              ))}
          ...
          <div>
            ...
            <ul className="flex flex-col gap-2 text-[15px] text-slate">
              {footerCities().map((c) => (
                <li key={c.slug}>
                  {c.tier === 1 ? (
                    <Link href={`/service-area/${c.slug}`} className="hover:text-charcoal hover:underline">
                      {c.name}
                    </Link>
                  ) : (
                    <Link href="/service-area" className="hover:text-charcoal hover:underline">
                      {c.name}
                    </Link>
                  )}
                </li>
        ...
        <div className="border-t border-hairline pt-5 text-[13px] text-slate">
