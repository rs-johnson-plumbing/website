...
for(const touch of [false,true])test.describe(touch?'Phone composer':'Desktop composer',()=>{
  ...
  test('Enter follows device conventions and long drafts remain editable',async({page})=>{
    ...
    await page.route('**/api/robo-ryan',route=>route.fulfill({json:{reply:'What would you like to understand?'}}));
    ...
    await input.fill('Help');await input.press('Shift+Enter');
    await expect(input).toHaveValue('Help\n');await expect(chat.locator('.rr-user')).toHaveCount(0);
    await input.fill('Help');await input.press('Enter');
    if(touch){await expect(input).toHaveValue('Help\n');await chat.getByRole('button',{name:'Send Message',exact:true}).click()}
    await expect(chat.locator('.rr-user')).toHaveText('Help');await expect(input).toHaveValue('');
